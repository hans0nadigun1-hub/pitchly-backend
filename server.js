...
// ══════════════════════════════════════════════════════════
// Publier une réponse directement sur Google My Business
app.post('/api/reviews/publish', async (req, res) => {
  const { restaurant_id, reply_text } = req.body;
  if (!restaurant_id || !reply_text)
    return res.status(400).json({ error: 'restaurant_id et reply_text requis' });
  try {
    const { data: resto } = await supabase.from('restaurants').select('*').eq('id', restaurant_id).single();
    if (!resto) return res.status(404).json({ error: 'Commerce non trouvé' });
    oauth2Client.setCredentials({
      access_token:  resto.access_token,
      refresh_token: resto.refresh_token,
    });
